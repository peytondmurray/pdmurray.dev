...
<Plot
    data={[
        {
            x: [0, 20, 40, 60, 80, 100],
            y: [41.7, 40.9, 39.8, 36.7, 36.1, 35.0],
            type: 'scatter',
        },
    ]}
    layout={{
        xaxis: {
            title: 'Voltage Below Nominal (mV)',
        },
        yaxis: {
...
Finally, I made the changes permanent by `systemctl enable intel-undervolt.service` and `systemctl
start intel-undervolt.service`. For a few minutes work, I was able to reduce the CPU power
