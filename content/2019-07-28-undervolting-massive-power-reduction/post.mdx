...
process extremely fast. Start out by opening up a new terminal and entering `# intel-undervolt
measure`, to show a persistent display of the current CPU temperatures, frequencies, and power
...
`System Agent`, or `Analog I/O` voltages. Apply the new voltage setting by typing `# intel-undervolt
apply`. The effect might not be immediately obvious unless a benchmark is running - I tested a
...
Finally, I made the changes permanent by `# systemctl enable intel-undervolt.service` and `#
systemctl start intel-undervolt.service`. For a few minutes work, I was able to reduce the CPU power
consumption by ~7 W, or ~16% (at 100% CPU load). Considering the fact that this power savings comes
at zero cost to performance, I'm really pleased with how it turned out. In principle this should
make the CPU less susceptible to thermal throttling, too.
